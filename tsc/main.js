...
function themeForUrl(url) {
    if (getTheme() === 'light')
        return 'light';
    if (store.get('darkArtistPages', true) === false && isArtistPage(url))
        return 'light';
    return 'dark';
}
...
async function init() {
    ...
    const resolveRelease = async (req) => {
    ...
    electron_1.ipcMain.handle('collection:tracklist', async (_e, req) => {
        try {
            ...
            if (devMode)
                console.log('[bcrpc] collection:tracklist ' + req.tralbumType + req.tralbumId + ' band=' + req.bandId + ' -> ' + tracks.length + ' tracks');
            if (!tracks.length)
                return { ok: false, error: 'no tracks' };
    ...
    electron_1.ipcMain.on('app:theme-for', (e, url) => { e.returnValue = themeForUrl(typeof url === 'string' ? url : ''); });
    ...
    electron_1.ipcMain.handle('settings:save', (_e, data) => {
        try {
            ...
            if (typeof data.darkArtistPages === 'boolean') {
                if (data.darkArtistPages !== (store.get('darkArtistPages', true) !== false))
                    themeChanged = true;
        ...
        wc.on('dom-ready', async () => {
            try {
                ...
                if (themeForUrl(wc.getURL()) === 'light')
                    return; // no darkreader in light mode / on exempt artist pages
