...
class BandcampApi {
    ...
    async fetchReleaseYear(q) {
        const id = toId(q.tralbumId);
        if (!id)
            return 0;
        const key = `${q.tralbumType}:${id}`;
        if (this.yearCache.has(key))
            return this.yearCache.get(key);
        const types = q.tralbumType === 't' ? ['t', 'a'] : ['a', 't'];
    ...
    primeYear(type, id, year) {
        if (id && year)
            this.yearCache.set(`${type}:${toId(id)}`, year);
    }
    getReleaseYear(type, id) {
    ...
    attemptUrls(type, tralbumId, bandId) {
        ...
        for (const u of [mobile, info]) {
            if (bandId)
                u.searchParams.set('band_id', bandId);
    ...
    async fetchRawFrom(url) {
        const session = this.getSession();
        if (!session)
            return null;
        try {
            const res = await session.fetch(url, { credentials: 'include' });
            if (!res.ok)
                return null;
    ...
    async fetchRaw(type, tralbumId, bandId) {
        if (!tralbumId)
            return null;
     ...
     */
    async resolveQueueForTrack(trackId, bandId) {
        ...
        const resolvedBand = toId(trackData?.band_id ?? trackData?.current?.band_id ?? trackData?.selling_band_id ?? bandId);
        if (albumId) {
     ...
     */
    async fetchTracksFromUrl(pageUrl) {
        const session = this.getSession();
        if (!session || !pageUrl)
            return [];
        let html = '';
        try {
            const r = await session.fetch(pageUrl, { credentials: 'include' });
            if (!r.ok)
                return [];
            html = await r.text();
        }
        ...
        if (!blob)
            return [];
        const type = (blob.item_type === 'track' || blob.tralbum_type === 't') ? 't' : 'a';
    ...
    async resolvePageUrl(q) {
        ...
        if (alb) {
            const type = q.tralbumType === 't' ? 't' : 'a';
    ...
    normalize(data, q) {
        ...
        const tralbumType = (data.item_type || data.tralbum_type || q.tralbumType) === 't' ? 't' : 'a';
        ...
        const artist = (current.artist || data.artist || data.tralbum_artist ||
            data.band_name || (data.band && data.band.name) || 'Bandcamp').toString().trim();
        ...
        const rawTracks = Array.isArray(data.trackinfo)
            ? data.trackinfo
    ...
    normalizeCollectionItem(it, redownloadUrls) {
        const type = (it.item_type === 'track' || it.tralbum_type === 't') ? 't' : 'a';
     ...
     */
    async fetchCollection(maxItems = 20000, onProgress) {
        const session = this.getSession();
        if (!session)
            return [];
        let fanId = '';
        ...
        for (let page = 0; page < 200 && out.length < maxItems; page++) {
            ...
            const redl = (data?.redownload_urls && typeof data.redownload_urls === 'object') ? data.redownload_urls : {};
            const added = [];
            for (const it of items) {
                const c = this.normalizeCollectionItem(it, redl);
                const key = c.tralbumType + c.tralbumId;
                if (!c.tralbumId || seen.has(key))
                    continue;
                seen.add(key);
                out.push(c);
                added.push(c);
            ...
            if (onProgress)
                onProgress(added, out.length, Math.max(total, out.length));
            ...
            if (!data?.more_available || !next || seenTokens.has(next))
                break;
            seenTokens.add(next);
            token = next;
        }
    ...
    async fetchDownloadFormats(downloadPageUrl) {
        const session = this.getSession();
        if (!session || !downloadPageUrl)
            return [];
        let html = '';
        try {
            const r = await session.fetch(downloadPageUrl, { credentials: 'include' });
            if (!r.ok)
                return [];
            html = await r.text();
        }
        ...
        for (const enc of Object.keys(downloads)) {
            ...
            if (dl && dl.url)
                out.push({ encoding: enc, label: (dl.description || enc).toString(), url: dl.url.toString() });
        }
        return out;
    }
    ...
    async prepareDownload(formatUrl) {
        const session = this.getSession();
        if (!session || !formatUrl)
            return formatUrl;
        const statUrl = formatUrl.replace('/download/', '/statdownload/') + '&.vrs=1&.rand=' + Math.floor(Math.random() * 1e9);
        for (let i = 0; i < 45; i++) {
            try {
                const r = await session.fetch(statUrl, { credentials: 'include' });
                const text = await r.text();
                const jm = text.match(/\{[\s\S]*\}/); // strip any jsonp wrapper
                if (jm) {
                    const j = JSON.parse(jm[0]);
                    if (j.result === 'ok' && (j.download_url || j.url))
                        return (j.download_url || j.url).toString();
                    if (j.result === 'err')
                        return formatUrl;
                }
            }
...
exports.BandcampApi = BandcampApi;
