...
class LastfmService {
    ...
    cfg() {
        const raw = this.store.get('lastfm') || {};
        return {
            apiKey: raw.apiKey || '',
    ...
    async beginAuth() {
        ...
        if (!c.apiKey || !c.apiSecret)
            return { error: 'Missing Last.fm API key/secret' };
        try {
     ...
     */
    async pollForSession(attempts = 60, intervalMs = 2500) {
    ...
    async completeAuth() {
        ...
        try {
            const data = await this.call({ method: 'auth.getSession', api_key: c.apiKey, token: this.pendingToken }, 'GET');
            const session = data.session;
            if (!session?.key)
                return { error: data.message || 'Authorization not confirmed yet' };
    ...
    async updateNowPlaying(track) {
        if (!this.isReady() || !track.isPlaying || !track.title || !track.artist)
            return;
        const key = track.id + '|' + track.title;
        if (key === this.lastNowPlaying)
            return;
